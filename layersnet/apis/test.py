...
from layersnet.datasets.utils import writePKL, to_numpy_detach
...
def single_gpu_rollout(model, data_loader, show=False, out_dir=None, **show_kwargs):
    ...
    for i, data in enumerate(data_loader):
        # The gt must contain the same states dim
        states_dim = data['gt_label']['vertices'].data[0].shape[1]
        ...
        if rollout_pointer == seq_num:
            ...
            if history_inputs is not None:
                current_state_inputs = torch.from_numpy(history_inputs.astype(np.float32)).unsqueeze(0)
            data = dataset.prepare_rollout(batch_data=data, current_garment=current_state_inputs)
        else:
            # Begin a new rollout
            rollout_pointer = seq_num
            cuda_history_inputs = data['inputs'].get('state', None)
            if cuda_history_inputs is not None:
                history_inputs = np.array(to_numpy_detach(data['inputs']['state'].data[0].transpose(-1, -2).squeeze(0)))
            pass
        ...
        assert len(data['inputs']['indices'].data[0]) == 1

        results.append({
            'rollout_idx': to_numpy_detach(rollout_pointer)[0],
            'acc': result['acc'],
            'indices': deepcopy(to_numpy_detach(data['inputs']['indices'].data[0][0])),
            'indices_type': deepcopy(to_numpy_detach(data['inputs']['indices_type'].data[0][0]))
            })
        
        if getattr(data_loader.dataset, 'supervised_rollout', False):
            prev_pred = np.array(to_numpy_detach(data['gt_label']['vertices'].data[0].transpose(-1, -2)[0]))
        else:
        ...
        if show or out_dir:
            ...
            for i in range(len(pred)):
                ...
                # Only need position predictions
                V = pred[i][:, :3]
        ...
        x = data['gt_label']['vertices'].data[0] if not isinstance(data['gt_label'], list) else data['gt_label'][0]['vertices']
        batch_size = x.size(0)
